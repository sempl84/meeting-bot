...
const joinTelemost = async (req: Request, res: Response) => {
  const {
    bearerToken,
    url,
    name,
  ...
  const logger = loggerFactory(correlationId, 'telemost');
  ...
  try {
    ...
    const jobResult = await globalJobStore.addJob(async () => {
      ...
      const namePrefix = getRecordingNamePrefix('telemost');
      ...
      const bot = new TelemostBot(logger, correlationId);
    ...
    logger.info('Telemost job accepted and started processing', { userId, teamId });

    return res.status(202).json({
      success: true,
      message: 'Telemost join request accepted and processing started',
      data: {
        userId,
        teamId,
        eventId,
    ...
    logger.error('Error setting up Telemost job:', { userId, teamId, botId, eventId, error });
